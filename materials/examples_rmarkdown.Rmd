---
title: "R Markdown Tutorial - examples"
...
Now the message doesn't display, but the code was still executed. Same applies to errors and warnings.

## eval, include, echo

If you want the code to appear in the output file, but for the **code to NOT run** when the Rmd file is knitted, set `eval = FALSE` as a code chunk option. For example, you may want to show the code you used to export the fish plot above as a jpg file, but to not have it exported every time you knit the file.

```{r export cpue plot, eval = FALSE}
ggsave(
  "CPUE_plot_month.jpg",
  plt_cpue_month,
  height = 5,
...
If you want the code to be executed when the Rmd file is knitted, but for the **code and results to NOT appear** in the output file, set `include = FALSE` as a code chunk option. For example, maybe you want to summarize the count and CPUE data as annual totals in a new data frame `df_annual` and plot the CPUE results, but not show the code you used or the plot in the document.

```{r summarize and plot annual count and cpue, include = FALSE}
...
Even though the code and results aren't displayed in the document, the `df_annual` data frame is available to be used within the Rmd file.

```{r glimpse annual data}
glimpse(df_annual)
```

If you want the code to executed and results to be displayed in the output file when the Rmd is knitted, but for the **code to NOT appear** in the output file, set `echo = FALSE` as a code chunk option. For example, maybe you want the plot of annual CPUE to display, but not show the code used to create it.

```{r plot annual cpue, echo = FALSE}
df_annual %>% 
  ggplot(aes(x = Year, y = CPUE)) +
  geom_col() +
  facet_wrap(vars(Gear), ncol = 1, scales = "free_y") + 
...
However, the kable formatting can be improved. The `knitr::kable()` function allows for some minor formatting such as rounding digits and column alignment.
...
df_annual %>%
  ...
  scroll_box(width = "35%", height = "600px")
```
<br>
...
A feature that I've found to be useful is using tabs in a rendered R Markdown document. Here is how you would set that up to display the two CPUE plots and the stations map in tabs for easier navigation.
...
Add `.tabset-fade` and/or `.tabset-pills` to change the appearance and behavior of the tabs
